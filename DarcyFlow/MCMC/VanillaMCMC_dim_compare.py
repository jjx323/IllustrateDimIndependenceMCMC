...
def sampling_with_different_dim(params):
    ...
    sampler = VanillaMCMC(model, beta=beta, reduce_chain=np.int64(1e3), save_path=None)
...
np.save(results_folder/"dims", dims)
np.save(results_folder/"betas", betas)
np.save(results_folder/"VanillaMCMC_dim_priorC", acc_rates)
...
for idx, dim in enumerate(dims):
    ...
    plt.xscale("log")
    plt.legend()
plt.savefig(results_folder/"VanillaMCMC_dim_independence.png")
plt.close()
