...
def sampling_with_different_dim(params):
    ...
    # param0.x.array[:] = 0.0
    ## generate truth and save
    ## Here, we employed the prior with covariance operator C = (0.5*I)^{-2},
    ## which is not allowed for the theoretical consideration.
    params_prior = {
        "theta": lambda x: 0.0 + 0.0*x[0],
...
# acc_rates = sampling_with_different_dim((dims[0], betas[0]))

acc_rates = np.array(acc_rates).reshape(len(betas), len(dims))
np.save(results_folder/"betas", betas)
np.save(results_folder/"pCN_dim_priorI", acc_rates)
...
for idx, dim in enumerate(dims):
    ...
    plt.xscale("log")
    plt.legend()
plt.savefig(results_folder/"pCN_dim_priorI.png")
plt.close()
